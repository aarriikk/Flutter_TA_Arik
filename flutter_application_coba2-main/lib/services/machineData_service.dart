...
class MachineDataService {
  Future<http.Response?> getDataPdf(
      {required BuildContext context, required machineId, required id}) async {
    try {
      ...
      http.Response res = await http.get(
          Uri.parse(
              '${Constants.uri}/calculation?machineId=${machineId}&id=${id}'),
  ...
  // Future<http.Response?> getDataRaspi(
  //     {required BuildContext context, required machineId, required id}) async {
  //   try {
  //     SharedPreferences prefs = await SharedPreferences.getInstance();
  //     String? token = prefs.getString('x-auth-token');
  ...
  //     http.Response res = await http.get(
  //         Uri.parse(
  //             '${Constants.uri}/raspberry?machineId=${machineId}&id=${id}'),
  //         headers: <String, String>{
  //           'Content-Type': 'application/json; charset=UTF-8',
  //           'x-auth-token': token!
  //         });
  ...
  Future<http.Response> getAllMachine(
      {required BuildContext context, required machineId}) async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      String? token = prefs.getString('x-auth-token');

      http.Response res = await http.get(
        Uri.parse('${Constants.uri}/raspberry?machineId=$machineId'),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
          'x-auth-token': token!
        },
  ...
  Future<http.Response> getAllMparam(
      {required BuildContext context, required machineId}) async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      String? token = prefs.getString('x-auth-token');
      ...
      http.Response res = await http.get(
        ...
        // Uri.parse('${Constants.uri}/parameter?machineId=1'),
        headers: <String, String>{
          'Content-Type': 'application/json; charset=UTF-8',
          'x-auth-token': token!
        },
  ...
  Future<http.Response?> getByIdRaspi(
      {required BuildContext context, required id}) async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      String? token = prefs.getString('x-auth-token');
      http.Response res = await http.get(
          Uri.parse('${Constants.uri}/raspberry/${id}'),
          headers: <String, String>{
            'Content-Type': 'application/json; charset=UTF-8',
            'x-auth-token': token!
          });
  ...
  void createMachineData(
      ...
      required String rawMaterialPrice}) async {
    try {
      SharedPreferences prefs = await SharedPreferences.getInstance();
      String? token = prefs.getString('x-auth-token');
